...
const RegisterSection = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await emailjs.send(serviceId, templateUser, {
      ...
      try {
        const thankYouWindow = window.open(`/thank-you?email=${encodeURIComponent(formData.email)}`, "_blank");
        if (thankYouWindow) thankYouWindow.focus();
      } catch {}

      setSuccessMessage(`${t("regSuccessMsg")} ${formData.email}`);
      setTimeout(() => {
  ...
  return (
    ...
    >
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        <motion.div className={`text-center mb-12 ${isRevealed ? 'scroll-reveal revealed' : 'scroll-reveal'}`}>
          <motion.div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass-advanced mb-6 animate-slide-up">
            ...
            <span className="text-sm font-mono-display text-primary font-bold">{t("limitedSeats")}</span>
          </motion.div>

          <h2 className="text-display-xl mb-6 leading-tight text-center">
            {t("registerTitle")}{" "}
            <span className="text-gradient-animated">{t("registerTitleHighlight")}</span>
          </h2>
          ...
          >
            {t("registerDesc")} <span className="text-primary font-semibold">{t("registerDescMid")}</span>{" "}
            {t("registerDescMid2")}{" "}
            <span className="text-accent font-semibold">{t("registerDescHighlight")}</span>.
            <br />
            {t("registerDescEnd")}
          </motion.p>
          ...
          >
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-accent mb-1 animate-scale-pulse">{t("registerTitleHighlight")}</div>
              <div className="text-xs text-muted-foreground">{t("workshop")}</div>
            </div>
            <div className="text-center">
        ...
        <motion.div
          className={`glass-advanced rounded-3xl p-8 md:p-12 text-center glow-primary interactive-card ${isRevealed ? 'scroll-scale revealed' : 'scroll-scale'}`}
          ...
          <div className="relative z-10">
            ...
            >
              <Dialog open={isDialogOpen} onOpenChange={(open) => {
                ...
                if (!open) {
                  setFormData({ name: "", email: "", designation: "", department: "", contact: "" });
                  setSuccessMessage("");
                  setError("");
                }
              }}>
                ...
                <DialogContent className="sm:max-w-[500px]">
                  <DialogHeader>
                    <DialogTitle className="text-center text-2xl font-bold">
                      {successMessage ? t("regSuccess") : t("regFormTitle")}
                    </DialogTitle>
                  </DialogHeader>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        <div>
                          ...
                          <Input id="name" type="text" placeholder={t("fullNamePlaceholder")} name="name" value={formData.name} onChange={handleChange} required />
                        </div>
                        <div>
                          <Label htmlFor="email">{t("emailAddress")} *</Label>
                          <Input id="email" type="email" placeholder={t("emailPlaceholder")} name="email" value={formData.email} onChange={handleChange} required />
                        </div>
                        <div>
                          <Label htmlFor="designation">{t("designation")} *</Label>
                          <Input id="designation" type="text" placeholder={t("designationPlaceholder")} name="designation" value={formData.designation} onChange={handleChange} required />
                        </div>
                        <div>
                          <Label htmlFor="department">{t("department")} *</Label>
                          <Input id="department" type="text" placeholder={t("departmentPlaceholder")} name="department" value={formData.department} onChange={handleChange} required />
                        </div>
                        <div>
                          <Label htmlFor="contact">{t("contactNumber")} *</Label>
                          <Input id="contact" type="tel" placeholder={t("contactPlaceholder")} name="contact" value={formData.contact} onChange={handleChange} pattern="[0-9+\-\s]+" required />
                        </div>
                      </div>
                      <Button type="submit" disabled={loading} className="w-full" size="lg">
                        {loading ? t("registering") : t("completeReg")}
                      </Button>
                    </form>
                  )}
                </DialogContent>
